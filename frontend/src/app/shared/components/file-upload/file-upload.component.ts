...
export class FileUploadComponent implements OnInit {
  //@Input() url = null;
  @Input() multiple =
    false; /* controlar tipo single o multuple propiedad del input tipo file */
  @Input() document_type =
    "archivos"; /* type document cotizacion, orden, factura etc... */
  @Input() comment = null;
  @Input() title =
    "Cargador Archivos"; /*Titulo por defecto de el cargador de archivos */
  @Input() data =
    {}; /* Datos extra para subir con los archivos para procesar */
  ...
  @Input()
  max_size: number = 0; /* Tamaño maximo de los archivos numero entero equivalente a MB */
  @Input() autoupload = false; /* subia automatica de archivos */
  @Input() url2: string = null; /*Url de entrada si se desea una url custom */
  @Output()
  ArchivosCargados: EventEmitter<Object>; /*Evento que emite los archivos exitosos */
  public archivos: any[] =
    []; /* array de archivos subidos se envia en el emit*/
  ...
  public urlapi: string =
    environment.apiUrl + "files/uploads"; /*Url por defecto del cargador */

  error_archivos: boolean =
    false; /* variable de control para validaciones de extensiones, tamaño y otras */
  elementos_borrados: string[] =
    []; /*elementos borrados de la seleccion porque no pasan las validaciones */
  ...
  getContadores() {
    ...
    for (let j = 0; j < this.uploader.queue.length; j++) {
      ...
      for (let k = 0; k < this.uploader.queue.length; k++) {
        if (
          this.uploader.queue[j].file.name == this.uploader.queue[k].file.name
        ) {
          repetido++;
        }
        for (let s = 0; s < retorno.length; s++) {
          if (
            retorno[s].name == this.uploader.queue[j].file.name &&
            add_retorno == true
          ) {
            add_retorno = false;
          }
      ...
      if (add_retorno == true) {
        retorno.push({
          name: this.uploader.queue[j].file.name,
          contador: repetido,
        });
      }
    }
  ...
  validarSize() {
    ...
    if (this.max_size > 0) {
      ...
      if (error) {
        alert(
          "los archivos con tamaño superior a " +
            this.max_size +
            " megas no son permitidos"
        );
      }
    }
  }
