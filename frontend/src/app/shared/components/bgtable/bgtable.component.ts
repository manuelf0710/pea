...
export class BgtableComponent implements OnInit {
  ...
  public formSearch: Array<object> =
    []; /*array objetos searchable in tableConfig */
  public paramSearch: any[] =
    []; /* array properties searchables in tableConfig ["nombre", "documento"] */
  public paramSearchToObject: object =
    {}; /*variables de array pasados a objeto, para enviar en la petición httpparams del servicio */
  ...
  iconExports(n) {
    ...
    else return code.icon;
  }

  pageChange(pag) {
    this.loadTableData(
      this.tableConfig.url + "?page=" + pag + "&pageSize=" + this.pageSize
    );
  }
  reloadTable() {
  ...
  public onChangePaginationSize() {
    this.currentPage = 1;
    this.loadTableData(
      this.tableConfig.url + "?page=1&pageSize=" + this.pageSize
    );
  }
  ...
  deleteRowtable(data) {
    ...
    this.dataSource = this.dataSource.filter(
      (obj) => obj[theKey] !== data[theKey]
    );
  }

  loadTableData(url) {
    this.isSearching = true;
    this._BgtableService.getLista(url, this.paramSearchToObject).subscribe(
      (res: any) => {
